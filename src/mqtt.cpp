  ...
  if (strcasecmp(msgCpy.topic, addTopic("/cmd/restart")) == 0) {
    ...
    if (channel >= 1 && channel <= 16) {
      if (strcasecmp(msgCpy.payload, "UP") == 0 || strcasecmp(msgCpy.payload, "OPEN") == 0 || strcmp(msgCpy.payload, "0") == 0) {
        ...
        // Numerische Payload = Zielposition in % ?
        char *endPtr;
        long posVal = strtol(msgCpy.payload, &endPtr, 10);
        if (endPtr != msgCpy.payload && *endPtr == '\0' && posVal >= 0 && posVal <= 100) {
          bool ok = shutterPosSetTarget(
              channel - 1, (int8_t)posVal,
          ...
          if (!ok) {
            mqttPublish(addTopic("/message"), "channel not calibrated", false);
            ESP_LOGW(TAG, "ch %d: position cmd ignored - not calibrated", channel);
          }
        } else {
